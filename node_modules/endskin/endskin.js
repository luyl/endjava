...
function EndSkin(tmpId)
{
	...
	this.getTemplateString = function(id)
	{
		...
		// 移除<%@ page language="java" ... %>
		ms = s.match(/<%@.*?page.*language=\"java\".*?%>/ig);
		if (ms) {
			for(i = 0; m = ms[i]; i++){
				s = s.replace(m,'');
			}
...
//					console.log("page:" + page);
		...
		if (ms = page.match(/\<c\:if.*?test=\"\$\{empty.*?([a-zA-Z\.]+).*?\}.*?\>/ig))
		{
			for(i=0; m=ms[i]; i++)
			{
				var _ms = m.match(/\<c\:if.*?test=\"\$\{empty.*?([a-zA-Z\.]+).*?\}.*?\>/i);
				if (_ms)
				{
					var variable = this._replace_var_name_jsp(_ms[1]);
